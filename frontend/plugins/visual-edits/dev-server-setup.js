...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*\.sh$/)) {
        return true;
      }

      // Allow all agent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*agent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@.sh" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@.sh" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
