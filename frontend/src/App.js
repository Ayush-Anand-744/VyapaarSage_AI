...
const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || 'https://vyapaarsage-ai-api.onrender.com';
const API = `${BACKEND_URL}/api`;
const GITHUB_PAGES_BASE = window.location.hostname.endsWith('github.io') ? '/VyapaarSage_AI' : '';
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="text-center text-gray-500 py-12">
              ...
              <p>Test the VyapaarSage_AI here!</p>
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const navItems = [
    ...
    { path: '/messages', icon: <MessageSquare />, label: 'AI Assistant' },
  ];
    ...
    <>
      ...
      >
        <div className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-white">VyapaarSage_AI</h1>
              <p className="text-amber-300 text-sm">SME Commerce Workspace</p>
            </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="lg:hidden bg-white shadow-sm border-b border-gray-200 px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-lg font-bold text-amber-900">VyapaarSage_AI</h1>
          <div className="w-6" />
        </div>
      </header>
      ...
      <footer id="vyapaarsage-owner-footer" className="fixed left-0 right-0 bottom-0 z-50 text-center px-4 py-2 text-xs text-white/80 bg-amber-950/80 backdrop-blur border-t border-white/10">
        © 2026 Ayush Anand · VyapaarSage_AI · All rights reserved.
      </footer>
    </div>
  );
};
...
function App() {
  return (
    <BrowserRouter basename={GITHUB_PAGES_BASE}>
      <AppLayout>
        <Routes>
